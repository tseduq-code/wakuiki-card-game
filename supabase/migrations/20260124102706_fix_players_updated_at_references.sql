/*
  # players テーブルのupdated_at参照を修正

  1. 問題
    - `players`テーブルには`updated_at`カラムが存在しない
    - 関数内で`updated_at`を更新しようとしてエラーが発生

  2. 修正内容
    - atomic_draw_card関数：players更新時の`updated_at = now()`を削除
    - atomic_discard_card関数：players更新時の`updated_at = now()`を削除
    - atomic_exchange_card関数：players更新時の`updated_at = now()`を削除

  3. 注意
    - `game_rooms`テーブルには`updated_at`が存在するため、そちらは維持
*/

-- atomic_draw_card関数を修正（players.updated_atを削除）
...
-- atomic_discard_card関数を修正（players.updated_atを削除）
...
-- atomic_exchange_card関数を修正（players.updated_atを削除）
